...
namespace Toggl.Foundation.Tests.Sync.States
{
    public sealed class PersistProjectsStateTests : PersistStateTests
    {
            ...
            : TheStartMethod<PersistProjectsState, IProject, IDatabaseProject>
        {
            ...
            protected override void SetupDatabaseBatchUpdateMocksToReturnDatabaseEntitiesAndFilterOutDeletedEntities(ITogglDatabase database, List<IProject> workspaces = null)
            {
                var foundationWorkspaces = workspaces?.Where(project => project.ServerDeletedAt == null).Select(Models.Project.Clean);
            ...
            {
                database.Projects.Received().BatchUpdate(Arg.Is<IEnumerable<IDatabaseProject>>(
                        list => list.Count() == projects.Count && list.All(shouldBePersistedAndIsClean(projects))),
